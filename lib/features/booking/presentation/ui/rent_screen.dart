...
class _RentScreenState extends State<RentScreen> {
    ...
    return BlocConsumer<AddBookingBloc, AddBookingState>(
      ...
      builder: (context, state) {
        return Scaffold(
          ...
          body: Padding(
            padding: const EdgeInsets.all(16.0),
            child: widget.car != null ? _buildCarDetails(state) : _buildNoCarDetails(),
          ),
        );
      },
    );
  ...
  Widget _buildCarDetails(AddBookingState state) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Image.network(widget.car!.url),
        ...
        Center(
          child: ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.kOxfordBlue),
            onPressed: state is AddBookingLoading ? null : _bookCar,
            child: state is AddBookingLoading 
  ...
  Future<void> _bookCar() async {
    if (_startDate != null && _endDate != null && widget.car != null) {
      ...
      if (userId != null) {
        ...
        toastInfo(message: 'User id not found');
      }
    } else {
      toastInfo(message: 'Please fill both dates');
    }
  }
}
